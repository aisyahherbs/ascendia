...
create table if not exists stockists (
  ...
  renewal_failed boolean not null default false,
...
alter table members add column if not exists renewal_failed boolean not null default false;
alter table members add column if not exists deleted_at timestamptz;
alter table members add column if not exists tupo_done boolean not null default false;
...
create table if not exists member_period_metrics (
  ...
  pair_left_basis_pv numeric(18,2) not null default 0,
  pair_right_basis_pv numeric(18,2) not null default 0,
  pair_volume_pv numeric(18,2) not null default 0,
  left_carry_pv numeric(18,2) not null default 0,
  right_carry_pv numeric(18,2) not null default 0,
  carry_in_left_pv numeric(18,2) not null default 0,
  carry_in_right_pv numeric(18,2) not null default 0,
  expired_carry_pv numeric(18,2) not null default 0,
  carry_tranches jsonb not null default '[]'::jsonb,
...
alter table member_period_metrics add column if not exists pair_left_basis_pv numeric(18,2) not null default 0;
alter table member_period_metrics add column if not exists pair_right_basis_pv numeric(18,2) not null default 0;
alter table member_period_metrics add column if not exists pair_volume_pv numeric(18,2) not null default 0;
alter table member_period_metrics add column if not exists left_carry_pv numeric(18,2) not null default 0;
alter table member_period_metrics add column if not exists right_carry_pv numeric(18,2) not null default 0;
alter table member_period_metrics add column if not exists carry_in_left_pv numeric(18,2) not null default 0;
alter table member_period_metrics add column if not exists carry_in_right_pv numeric(18,2) not null default 0;
alter table member_period_metrics add column if not exists expired_carry_pv numeric(18,2) not null default 0;
alter table member_period_metrics add column if not exists carry_tranches jsonb not null default '[]'::jsonb;
...
values ('menu_visibility', '{"admin":{},"branch":{},"stockist":{},"member":{}}'::jsonb)
on conflict (key) do nothing;

do $$
begin
  if exists (select 1 from pg_constraint where conname = 'members_sponsor_id_fkey') then
    alter table members drop constraint members_sponsor_id_fkey;
  end if;
  alter table members add constraint members_sponsor_id_fkey
    foreign key (sponsor_id) references members(id) on update cascade on delete restrict;

  if exists (select 1 from pg_constraint where conname = 'members_placement_parent_id_fkey') then
    alter table members drop constraint members_placement_parent_id_fkey;
  end if;
  alter table members add constraint members_placement_parent_id_fkey
    foreign key (placement_parent_id) references members(id) on update cascade on delete restrict;

  if exists (select 1 from pg_constraint where conname = 'members_stockist_id_fkey') then
    alter table members drop constraint members_stockist_id_fkey;
  end if;
  alter table members add constraint members_stockist_id_fkey
    foreign key (stockist_id) references stockists(id) on update cascade on delete restrict;

  if exists (select 1 from pg_constraint where conname = 'sponsor_closure_ancestor_id_fkey') then
    alter table sponsor_closure drop constraint sponsor_closure_ancestor_id_fkey;
  end if;
  alter table sponsor_closure add constraint sponsor_closure_ancestor_id_fkey
    foreign key (ancestor_id) references members(id) on update cascade on delete cascade;

  if exists (select 1 from pg_constraint where conname = 'sponsor_closure_descendant_id_fkey') then
    alter table sponsor_closure drop constraint sponsor_closure_descendant_id_fkey;
  end if;
  alter table sponsor_closure add constraint sponsor_closure_descendant_id_fkey
    foreign key (descendant_id) references members(id) on update cascade on delete cascade;

  if exists (select 1 from pg_constraint where conname = 'placement_closure_ancestor_id_fkey') then
    alter table placement_closure drop constraint placement_closure_ancestor_id_fkey;
  end if;
  alter table placement_closure add constraint placement_closure_ancestor_id_fkey
    foreign key (ancestor_id) references members(id) on update cascade on delete cascade;

  if exists (select 1 from pg_constraint where conname = 'placement_closure_descendant_id_fkey') then
    alter table placement_closure drop constraint placement_closure_descendant_id_fkey;
  end if;
  alter table placement_closure add constraint placement_closure_descendant_id_fkey
    foreign key (descendant_id) references members(id) on update cascade on delete cascade;

  if exists (select 1 from pg_constraint where conname = 'pv_transactions_member_id_fkey') then
    alter table pv_transactions drop constraint pv_transactions_member_id_fkey;
  end if;
  alter table pv_transactions add constraint pv_transactions_member_id_fkey
    foreign key (member_id) references members(id) on update cascade on delete restrict;

  if exists (select 1 from pg_constraint where conname = 'member_period_metrics_member_id_fkey') then
    alter table member_period_metrics drop constraint member_period_metrics_member_id_fkey;
  end if;
  alter table member_period_metrics add constraint member_period_metrics_member_id_fkey
    foreign key (member_id) references members(id) on update cascade on delete cascade;

  if exists (select 1 from pg_constraint where conname = 'bonus_ledger_member_id_fkey') then
    alter table bonus_ledger drop constraint bonus_ledger_member_id_fkey;
  end if;
  alter table bonus_ledger add constraint bonus_ledger_member_id_fkey
    foreign key (member_id) references members(id) on update cascade on delete restrict;

  if exists (select 1 from pg_constraint where conname = 'bonus_ledger_source_member_id_fkey') then
    alter table bonus_ledger drop constraint bonus_ledger_source_member_id_fkey;
  end if;
  alter table bonus_ledger add constraint bonus_ledger_source_member_id_fkey
    foreign key (source_member_id) references members(id) on update cascade on delete restrict;

  if exists (select 1 from pg_constraint where conname = 'member_bonus_payments_member_id_fkey') then
    alter table member_bonus_payments drop constraint member_bonus_payments_member_id_fkey;
  end if;
  alter table member_bonus_payments add constraint member_bonus_payments_member_id_fkey
    foreign key (member_id) references members(id) on update cascade on delete restrict;

  if exists (select 1 from pg_constraint where conname = 'stockist_payouts_stockist_id_fkey') then
    alter table stockist_payouts drop constraint stockist_payouts_stockist_id_fkey;
  end if;
  alter table stockist_payouts add constraint stockist_payouts_stockist_id_fkey
    foreign key (stockist_id) references stockists(id) on update cascade on delete restrict;
end $$;
