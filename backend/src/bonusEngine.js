...
function buildContext(rows, settingsRows) {
  ...
  const members = rows.map((row) => ({
    ...
    rankName: row.metric_rank_name || row.rank_name || "Member",
...
export async function runBonusCalculation(client, periodKey, actorId) {
  ...
  const rawPairTotal = [...rawPair.values()].reduce((sum, value) => sum + value, 0);

  const rawMentoringLedger = mentoringLedger(context, rawPair);
  ...
  const rawPairSystemBv = rawPairTotal + sumLedger(rawMentoringLedger);
  const availablePairSystemBv = Math.max(0, maxPayoutBv - fixedPayoutBv);
  const pairFactor = rawPairSystemBv > availablePairSystemBv && rawPairSystemBv > 0
  ...
  context.members.forEach((member) => {
    ...
    addLedger(
      ...
      null,
      null,
      pairFactor >= 1 ? "Bonus Pasangan dari kaki terlemah" : `Bonus Pasangan disesuaikan faktor ${Math.round(pairFactor * 10000) / 100}%`
    );
  });
