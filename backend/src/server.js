...
async function findAvailablePlacementSlot(client, startId) {
  const queue = [startId];
  ...
  while (queue.length) {
    ...
    const children = await client.query(
      `
        ...
        from members
        where placement_parent_id = $1 and (active = true or renewal_failed = true)
...
async function assignPlacementSlot(client, preferredParentId) {
  if (!preferredParentId) return { parentId: null, side: null };
  const root = await client.query("select id from members where id = $1 and active = true", [preferredParentId]);
  if (!root.rowCount) throw Object.assign(new Error("Placement parent not found"), { status: 400 });
  const slot = await findAvailablePlacementSlot(client, preferredParentId);
...
async function repairPlacementIntegrity(client) {
  ...
  const parents = await client.query(
    `
      ...
      from members
      where active = true and placement_parent_id is not null
  ...
  for (const parent of parents.rows) {
    const children = await client.query(
      `
        select id, placement_side
        from members
        where active = true and placement_parent_id = $1
        order by placement_side nulls last, joined_at, id
      `,
      [parent.parent_id]
    );
