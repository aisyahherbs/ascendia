  ...
  const result = await query(
    `
      select m.id, m.name, m.rank_name, m.sponsor_id, m.placement_parent_id, m.placement_side,
             m.stockist_id, m.joined_at, m.tupo_done, m.tupo_blocked, pm.ppv, pm.appv, pm.tnpv, pm.atnpv, pm.gpv,
  ...
  const created = await withTransaction(async (client) => {
    ...
    const result = await client.query(
      `
        ...
        returning *
      `,
      [input.id, input.name, input.email || null, input.phone || null, sponsorId, input.placementParentId || null, input.placementSide || null, stockistId, input.rankName || "Member", input.password || "ascendia"]
    );
    await client.query(
