...
app.post("/api/members", requireOperator, asyncRoute(async (req, res) => {
  ...
  const created = await withTransaction(async (client) => {
    ...
    let placementParentId = input.placementParentId || null;
    let placementSide = input.placementSide || null;
    if (placementParentId) {
      ...
      if (!placementSide) {
        const slot = await findAvailablePlacementSlot(client, placementParentId);
        if (!slot || slot.parentId !== placementParentId) throw Object.assign(new Error("Placement parent is full"), { status: 400 });
