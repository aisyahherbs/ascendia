...
function rankFromAtnpv(atnpv) {
  ...
  if (atnpv >= 15000) return "Royal Star";
  if (atnpv >= 6000) return "VIP";
  return "Member";
}
...
async function recomputePeriodMetrics(client, periodKey) {
  ...
  for (const member of members) {
    ...
    const metricRank = rankFromAtnpv(atnpv);
    await client.query(
      `
  ...
  const result = await query(
    `
      select m.id, m.name, m.rank_name, m.sponsor_id, m.placement_parent_id, m.placement_side,
             m.stockist_id, m.joined_at, m.tupo_done, m.tupo_blocked,
  ...
  const result = await query(
    `
      select m.id, m.name, m.rank_name, m.sponsor_id, m.stockist_id, m.joined_at,
             m.tupo_done, m.tupo_blocked, pm.ppv, pm.appv, pm.tnpv, pm.atnpv, pm.gpv,
             (select count(*)::int from members child where child.sponsor_id = m.id and child.active = true) as sponsor_child_count,
             (select count(*)::int from members child where child.placement_parent_id = m.id and child.active = true) as placement_child_count
      from members m
      left join member_period_metrics pm on pm.member_id = m.id and pm.period_key = $${periodIndex}
  ...
  const result = await query(
    `
      select m.id, m.name, m.rank_name, m.sponsor_id, m.stockist_id,
             m.placement_parent_id, m.placement_side, m.joined_at, m.tupo_done, m.tupo_blocked,
             pm.ppv, pm.appv, pm.tnpv, pm.atnpv, pm.gpv,
             (select count(*)::int from members child where child.sponsor_id = m.id and child.active = true) as sponsor_child_count,
             (select count(*)::int from members child where child.placement_parent_id = m.id and child.active = true) as placement_child_count
      from members m
    ...
    const result = await client.query(
      `
        ...
        set
          ...
          disable_bonus_performance = $12,
          disable_bonus_pair = $13,
          disable_bonus_leadership = $14,
          disable_bonus_mentoring = $15,
          disable_bonus_sharing = $16,
          updated_at = now()
        where id = $1
        returning *
      `,
