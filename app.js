...
function currentUser() {
  if (apiSession?.user && apiSession.user.id === state.activeId) {
    return {
      ...
      role: roleName(apiUserToLocalRole(apiSession.user.role)),
      area: apiSession.user.area || "",
      district: apiSession.user.district || "",
      rank: apiSession.user.rank || "Member",
...
function calcRawPairBonus(member) {
  ...
  const pairedPv = Math.min(member.leftPv || 0, member.rightPv || 0);
...
function pairPoolInfo(members = calcMembers()) {
  ...
  const pairBonusFactor = rawPairSystemBv > availablePairSystemBv && rawPairSystemBv > 0
    ? Math.max(0, Math.min(1, availablePairSystemBv / rawPairSystemBv))
    : 1;
  const maxPairPoolBv = pairBonusFactor >= 1 ? rawPairBonusBv : Math.floor(rawPairBonusBv * pairBonusFactor);
...
function calcPairSources(member) {
  ...
  if (!amount) return [];
  const rawAmount = calcRawPairBonus(member);
  const { pairBonusFactor, maxPairPoolBv, rawPairBonusBv, availablePairSystemBv } = pairPoolInfo();
  const poolNote = pairBonusFactor < 1
    ? `; hak mentah ${bv(rawAmount)} disesuaikan ruang payout pasangan ${bv(availablePairSystemBv)} dari total hak mentah pasangan ${bv(rawPairBonusBv)}`
    : "";
  return [{
    label: "Kaki kiri dan kanan",
    amount,
    note: `${rankRate(member, "pair")}% dari kaki terlemah ${pv(Math.min(member.leftPv || 0, member.rightPv || 0))}${poolNote}`
  }];
}
...
function render() {
  ...
  const activeNav = navByRole[state.activeRole].find(([key]) => key === activeView) || navByRole[state.activeRole][0];
  pageTitle.textContent = activeNav[1];
  nav.innerHTML = navByRole[state.activeRole].map(([key, label, icon]) => `
...
function recomputeMetrics(members = state.members) {
  ...
  const placementLegVolumesForMonth = (member, periodMonth) => {
    const directPlacement = placementChildrenOf(member.id).filter((child) => memberActiveInMonth(child, periodMonth));
    return {
      left: directPlacement[0] ? placementTeamPpvForMonth(directPlacement[0], periodMonth) : 0,
      right: directPlacement[1] ? placementTeamPpvForMonth(directPlacement[1], periodMonth) : 0
    };
  };
  ...
  const placementCarryInfo = (member, month = currentCalcMonth) => {
    if (!currentCalcMonth) {
      ...
      const leftPv = leftRoot ? Number(leftRoot.ppv || 0) + placementTeamPpv(leftRoot) : 0;
      const rightPv = rightRoot ? Number(rightRoot.ppv || 0) + placementTeamPpv(rightRoot) : 0;
...
function dashboardView() {
  ...
  const pairStatus = bonusTotals.rawPair <= 0
    ...
    : pairPool.pairBonusFactor >= 1
      ? "Bonus Pasangan dibayar maksimal karena payout masih dalam acuan 55%"
      : overPairPoolRupiah > 0
      ? "Bonus Pasangan disesuaikan karena simulasi full melewati acuan 55%"
      : pairIsMainCause
      ? "Bonus Pasangan adalah sumber payout terbesar"
      : "Bonus Pasangan ikut membentuk payout, tapi bukan sumber terbesar";
  ...
  return `
    ...
    <details class="card detail-card" style="margin-top:16px">
      ...
      <div class="bonus-grid">
        ...
        <div class="bonus-row"><span>Ruang Bayar Bonus Pasangan</span><b>${money(maxPairPoolRupiah)}</b></div>
        <div class="bonus-row"><span>Bonus Pasangan Dibayar</span><b>${money(bonusTotals.pair)}</b></div>
        <div class="bonus-row"><span>Pasangan Tertahan karena 55%</span><b>${heldPairRupiah ? money(heldPairRupiah) : "Tidak ada"}</b></div>
        <div class="bonus-row"><span>Ruang Payout Bonus Pasangan</span><b>${money(availablePairSystemRupiah)}</b></div>
      ...
      <p class="notice">Rasio ${payoutRatio}% dihitung dari Total Dibayar ${money(totalPayout)} dibagi TNPV Perusahaan ${money(companyTnpvRupiah)}. Bonus Pasangan sekarang dibayar maksimal selama ruang payout pasangan masih masuk acuan 55%; dashboard tetap menampilkan total payout keseluruhan agar admin bisa audit jika komponen lain ikut membesar.</p>
...
function serverDashboardView() {
  ...
  `).join("");
  return `
    ${controlPanel("Filter Dashboard Database", controls, `Ringkasan ${monthLabel(dashboardMonth)} dari PostgreSQL`)}
    <div style="height:16px"></div>
    ${serverDashboardSummary.loading ? `<article class="card"><p class="muted">Memuat dashboard database...</p></article>` : serverDashboardSummary.error ? `<article class="card"><p class="muted">${html(serverDashboardSummary.error)}</p></article>` : `
      <div class="grid kpi-grid">
        ...
        ${kpiCard("Bonus Member", money(data.memberBonusRupiah || 0), "Dari bonus ledger backend")}
        ...
        ${kpiCard("Periode", monthLabel(data.periodKey || dashboardMonth), "Mode database")}
      </div>
      <article class="card" style="margin-top:16px">
        <div class="toolbar">
          <div>
            <h3>Komponen Pembayaran</h3>
            <p class="muted">Bonus muncul setelah proses tutup buku backend mengisi bonus ledger.</p>
          </div>
        </div>
        <div class="table-wrap">
          <table>
...
function serverDashboardEnabled() {
  return typeof fetch === "function" && apiConnected() && ["admin", "branch", "stockist"].includes(state.activeRole) && adminReportScopeValue() !== "year";
}
...
async function loadServerDashboardSummary(key) {
  try {
    const period = state.dashboardMonth || state.selectedMonth || currentMonthValue();
    ...
    if (serverDashboardKey() !== key) return;
    serverDashboardSummary = { key, data: null, loading: false, error: error.message || "Dashboard database belum bisa dimuat." };
    if (error.status === 401) {
      isAuthenticated = false;
      saveApiSession(null);
      sessionStorage.removeItem(SESSION_KEY);
...
async function loadServerBonusSettings() {
  try {
    ...
    serverBonusSettingsState = { loaded: false, loading: false, error: error.message || "Pengaturan bonus database belum bisa dimuat." };
  } finally {
...
async function loadServerBonusPage(key) {
  try {
    ...
    serverBonusPage.error = error.message || "Data bonus database belum bisa dimuat.";
  } finally {
    serverBonusPage.loading = false;
    if (["bonuses", "payouts"].includes(activeView)) render();
  }
}
...
async function loadServerStockistPayouts(scope, key) {
  try {
    ...
    if (["stockists", "payouts"].includes(activeView)) render();
  }
}
...
async function loadServerCompanyFunds(key) {
  try {
    const result = await apiRequest(`/api/reports/company-funds?period=${encodeURIComponent(state.selectedMonth || currentMonthValue())}`);
    serverCompanyFunds = { key, data: result, loading: false, error: "" };
  } catch (error) {
    serverCompanyFunds = { key, data: null, loading: false, error: error.message || "Dana kelola database belum bisa dimuat." };
  } finally {
...
async function runServerBonusPeriod() {
  ...
  if (!confirm(`Jalankan tutup buku bonus backend untuk ${monthLabel(period)}? Hasil bonus ledger periode ini akan dihitung ulang.`)) return;
  ...
  try {
    ...
    serverBonusRun = {
      loading: false,
      message: `Tutup buku selesai: ${result.ledgerRows || 0} baris bonus, ${result.memberCount || 0} member.`,
      error: ""
    };
    ...
    showToast("Tutup buku backend selesai.", "success");
  } catch (error) {
    serverBonusRun = { loading: false, message: "", error: error.message || "Tutup buku backend gagal." };
...
async function loadServerMasterPage(type, key) {
  ...
  try {
    ...
    page.error = error.message || "Data database belum bisa dimuat.";
    if (error.status === 401) {
      isAuthenticated = false;
      saveApiSession(null);
      sessionStorage.removeItem(SESSION_KEY);
...
async function loadServerMembersPage(key) {
  try {
    ...
    serverMembersPage.error = error.message || "Data member database belum bisa dimuat.";
    if (error.status === 401) {
      isAuthenticated = false;
      saveApiSession(null);
      sessionStorage.removeItem(SESSION_KEY);
...
async function loadServerTupoPage(key) {
  try {
    ...
    serverTupoPage.error = error.message || "Data TUPO database belum bisa dimuat.";
    if (error.status === 401) {
      isAuthenticated = false;
      saveApiSession(null);
      sessionStorage.removeItem(SESSION_KEY);
...
function serverMembersView() {
  ...
  const rows = serverMembersPage.rows.map((member) => `
    <tr>
      <td><b>${html(member.id)}</b><br><span class="muted">${html(member.name)}</span></td>
      <td>${rankLabelHtml(member.rank)}</td>
      ...
      <td class="actions">
        ...
        <button class="icon-btn danger" title="Arsipkan" data-server-delete-member="${attr(member.id)}">X</button>
      </td>
    </tr>
  `).join("");
  return `
    ...
    ], "Mode database aktif: data dimuat 50 member per halaman agar ringan untuk banyak member.")}
    <div style="height:16px"></div>
    <article class="card">
      <div class="toolbar">
        <h3>Data Member Database</h3>
        <div class="toolbar-actions">
          ...
          ${state.activeRole !== "member" ? `<button class="primary" data-server-add-member>Tambah Member Database</button>` : ""}
        </div>
      </div>
      ${serverMembersPage.loading ? `<p class="notice">Memuat data member dari database...</p>` : ""}
      ${serverMembersPage.error ? `<p class="notice">${html(serverMembersPage.error)}</p>` : ""}
      <div class="table-wrap">
        <table>
          <thead><tr><th>Member</th><th>Peringkat</th><th>Tanggal Gabung</th><th>PPV / APPV</th><th>TNPV / ATNPV</th><th>Stokis</th><th>Aksi</th></tr></thead>
          <tbody>${rows || emptyRow(serverMembersPage.loading ? "Memuat data..." : "Tidak ada data member dari database.")}</tbody>
        </table>
      </div>
      <p class="notice">Mode database aktif. Tambah member dan input PV sudah masuk backend. Edit/hapus database disambungkan pada tahap berikutnya dengan audit log.</p>
    </article>
  `;
}
...
function serverTupoView() {
  ...
  const rows = members.map((member) => {
    ...
    return `
      <tr>
        <td class="select-col">
          <input type="checkbox" data-server-tupo-toggle="${attr(member.id)}" ${checked ? "checked" : ""} ${requiredPv > 0 && canEditAll() ? "" : "disabled"} title="${attr(requiredPv > 0 ? "Ceklis TUPO database" : "Tidak wajib TUPO")}">
        </td>
  ...
  return `
    ${controlPanel("Filter TUPO Bulanan", [monthControl("Bulan & Tahun")], "Mode database aktif: TUPO dibaca dari data VPS, bukan data lama browser.")}
    <div style="height:16px"></div>
    <div class="grid kpi-grid">
    ...
    <article class="card" style="margin-top:16px">
      ...
      ${serverTupoPage.loading ? `<p class="notice">Memuat TUPO dari database...</p>` : ""}
      ...
      <div class="table-wrap">
        <table>
          ...
          <tbody>${rows || emptyRow(serverTupoPage.loading ? "Memuat data..." : "Belum ada member aktif di database.")}</tbody>
        </table>
      </div>
      <p class="notice">Jika halaman ini kosong, berarti member belum masuk ke database VPS. Tambahkan member dari halaman Member Database.</p>
    </article>
  `;
}
  ...
  const rows = page.rows.map((item) => {
    ...
    return `
      <tr>
        ...
        <td>
          <label class="select-all-inline">
            <input type="checkbox" data-stockist-fee-toggle="${attr(item.id)}" data-server-stockist-transfer="true" data-sales-rupiah="${attr(payout.sales || 0)}" data-fee-rupiah="${attr(payout.fee || 0)}" data-member-bonus-rupiah="${attr(payout.memberBonus || 0)}" ${payout.transferPaid ? "checked" : ""} ${totalTransfer > 0 && scope !== "year" ? "" : "disabled"} title="${payout.transferPaid ? "Dana bonus member + fee sudah dikirim ke stokis" : "Tandai dana bonus member + fee sudah dikirim ke stokis"}">
            ${totalTransfer <= 0 ? "-" : payout.transferPaid ? "Sudah" : "Belum"}
          </label>
        </td>
        <td class="actions">${serverMasterActionButtons("stockists", item.id)}</td>
      </tr>
    `;
  }).join("");
  return `
    ${controlPanel("Filter Stokis", [
      ...
      stockistPaymentScopeControl("Tampilan"),
      scope === "payment" ? paymentPeriodControl("Periode Bayar") : ""
    ], serverStockistPayoutsEnabled() ? `Sumber ledger: ${source.label}` : "Mode database data master")}
    <div style="height:16px"></div>
    <article class="card">
      <div class="toolbar">
        <div>
          <h3>${state.activeRole === "branch" ? `Stokis Cabang ${html(currentUser()?.area || "")}` : "Data Stokis Database"}</h3>
          <p class="muted">Mode database: data stokis dimuat 50 baris per halaman, angka pembayaran diambil dari bonus ledger.</p>
        </div>
        <div class="toolbar-actions">
          ${searchBox("stockists", "ID, nama, provinsi, daerah")}
      ...
      <div class="table-wrap">
        <table>
          ...
          <tbody>${page.loading ? emptyRow("Memuat data stokis database...") : page.error ? emptyRow(page.error) : rows || emptyRow("Tidak ada stokis yang cocok.")}</tbody>
        </table>
      </div>
      ...
      <p class="notice">Checklist dana ke stokis berarti admin/cabang sudah mengirim dana bonus member ditambah fee stokis ke stokis tersebut.</p>
    </article>
  `;
}
...
function serverForfeituresView() {
  ...
  return `
    ...
    ], `Sumber database: ${selectedMonthLabel()}`)}
    <div style="height:16px"></div>
    ${serverCompanyFunds.loading ? `<article class="card"><p class="muted">Memuat dana kelola database...</p></article>` : serverCompanyFunds.error ? `<article class="card"><p class="muted">${html(serverCompanyFunds.error)}</p></article>` : `
      <div class="grid kpi-grid">
        ${kpiCard("Total untuk Perusahaan", money(data.totalCompanyRupiah || 0), "45% dasar + dana kelola")}
        ${kpiCard("Bagian Perusahaan 45%", money(data.companyBaseRupiah || 0), `Dari Omset/TNPV ${money(data.companyTnpvRupiah || 0)}`)}
        ${kpiCard("Dana Kelola", money(data.managedRupiah || 0), "Selisih pool bonus tidak terpakai")}
        ${kpiCard("Bonus Member", money(data.memberBonusRupiah || 0), "Dari bonus ledger")}
      ...
      <article class="card" style="margin-top:16px">
        <div class="toolbar">
          <div>
            <h3>Total Dana Kelola Database</h3>
            <p class="muted">Ringkasan ini dihitung dari bonus ledger setelah tutup buku backend.</p>
          </div>
        </div>
        <div class="table-wrap">
          <table>
            ...
            <tbody>
              <tr><td><b>Bagian Perusahaan 45%</b></td><td class="money">${money(data.companyBaseRupiah || 0)}</td><td>Dasar perusahaan dari Omset/TNPV.</td></tr>
...
function forfeituresView() {
  ...
  const companyManagedNote = audit.totals.activeCarryReserve > 0
    ? `Termasuk personal ${money(priorityManaged)}; dikurangi carry aktif ${money(audit.totals.activeCarryReserve)}${audit.poolSummary.inactiveReserved ? ` dan nonaktif massal ${money(audit.poolSummary.inactiveReserved)}` : ""}`
    : audit.poolSummary.inactiveReserved
      ? `Termasuk personal ${money(priorityManaged)}; dikurangi nonaktif massal ${money(audit.poolSummary.inactiveReserved)}`
  ...
  return `
    <article class="card">
      <div class="toolbar">
        <div>
          <h3>Pengumuman & Kebijakan Perusahaan</h3>
          <p class="muted">Mode database: pengumuman dimuat dari server.</p>
        </div>
        <div class="toolbar-actions">
          ${searchBox("announcements", "Reward, kebijakan, tanggal")}
      ...
      <div class="announcement-list">
        ${page.loading
          ? `<p class="muted">Memuat pengumuman database...</p>`
          : page.error
...
function serverPayoutsView() {
  ...
  return `
    ${controlPanel("Filter Komisi Stokis", [
      ...
      stockistPaymentScopeControl("Tampilan"),
      scope === "payment" ? paymentPeriodControl("Periode Bayar") : ""
    ], `Sumber ledger: ${source.label}`)}
    <div style="height:16px"></div>
    <div class="grid kpi-grid">
      ${kpiCard("Stokis", report.name, stockistLocationText(report))}
      ${kpiCard("Member Dilayani", report.memberCount || serverBonusPage.rows.length, scope === "payment" ? "Pembayaran bonus member" : "Ringkasan ledger")}
      ...
      ${kpiCard("Bonus Member", money(report.memberBonus), "Dari bonus ledger")}
      ...
      ${scope === "payment" ? kpiCard("Belum Dibayar", money(unpaidMemberBonus), "Perlu dicairkan ke member") : kpiCard("Sumber Data", source.label, "Database")}
    ...
    <article class="card" style="margin-top:16px">
      ...
      <div class="table-wrap">
        <table>
          ...
          <tbody>${serverBonusPage.loading ? emptyRow("Memuat bonus member database...") : serverBonusPage.error ? emptyRow(serverBonusPage.error) : rows || emptyRow("Tidak ada pembayaran member yang cocok.")}</tbody>
        </table>
      </div>
      <p class="notice">Data ini berasal dari bonus ledger setelah admin pusat menjalankan tutup buku backend.</p>
    </article>
  `;
}
  ...
  return `
    <article class="card">
      <div class="toolbar">
        <div>
          <h3>Pengelola Pusat & Cabang</h3>
          <p class="muted">Mode database: sandi tidak ditampilkan. Ubah sandi lewat tombol edit jika diperlukan.</p>
        </div>
        <div class="toolbar-actions">
          ${searchBox("admins", "ID, nama, role, area")}
      ...
      <div class="table-wrap">
        <table>
          ...
          <tbody>${page.loading ? emptyRow("Memuat data admin database...") : page.error ? emptyRow(page.error) : rows || emptyRow("Tidak ada admin yang cocok.")}</tbody>
        </table>
      </div>
      <div class="toolbar" style="margin-top:12px">
        <span class="pill">${page.rows.length} data tampil</span>
...
async function loadServerTreeRoot(kind, key) {
  ...
  try {
    ...
    if (serverTreeState[kind].key !== key) return;
    tree.error = error.message || "Struktur database belum bisa dimuat.";
    if (error.status === 401) {
      isAuthenticated = false;
      saveApiSession(null);
      sessionStorage.removeItem(SESSION_KEY);
...
async function loadServerTreeChildren(kind, memberId, shouldRender = true, key = serverTreeKey(kind)) {
  const tree = serverTreeState[kind];
  if (!memberId || tree.children.has(memberId) || tree.loadingChildren.has(memberId)) return;
...
function serverPlacementNodeMetrics(member, depth = 0, children = []) {
  const left = children.find((child) => child.placementSide === "L") || children[0] || null;
  const right = children.find((child) => child.placementSide === "R") || children.find((child) => child.id !== left?.id) || null;
  const recommendedSide = !left ? "Kiri" : !right ? "Kanan" : "Penuh";
  return `
    ...
    <div class="team-main placement-main">
      ...
      <div class="placement-leg-summary">
        ${placementMiniLeg("Kiri", left, left ? left.tnpv : 0, recommendedSide === "Kiri")}
        ${placementMiniLeg("Kanan", right, right ? right.tnpv : 0, recommendedSide === "Kanan")}
      </div>
    </div>
  `;
}
...
function serverTreeNodeHtml(kind, memberId, depth = 0, visited = new Set()) {
  ...
  const metrics = kind === "placement"
    ? serverPlacementNodeMetrics(member, depth, children)
...
function serverNetworkView() {
  ...
  return `
    ${controlPanel("Filter Jaringan", [monthControl("Bulan & Tahun")], "Mode database: generasi 1 dimuat otomatis, cabang berikutnya dibuka manual dengan tombol plus.")}
    <div style="height:16px"></div>
...
function serverPlacementView() {
  ...
  return `
    ${controlPanel("Filter Placement", [monthControl("Bulan & Tahun")], "Mode database: generasi 1 dimuat otomatis, kaki berikutnya dibuka manual dengan tombol plus.")}
    <div style="height:16px"></div>
...
function rankBadgeClass(rankName) {
  ...
  if (rankIndex[rankName] >= rankIndex["Royal Star"]) return "star";
  ...
  return `
    ...
    <article class="card" style="margin-top:16px">
      ...
      <div class="rules-table">
        ...
        <div class="rule-item"><b>Bonus Pasangan 15%</b><p>${isMemberRules ? "Mulai Royal Star 12%, Crown Star 13%, Leader Ambassador 14%, Leader Majestic sampai Executive Director 15%. Hak bonus dihitung dari kaki terlemah: rate rank x PV terpasang/lebih kecil, maksimal Rp250 juta per member per bulan." : "Mulai Royal Star 12%, Crown Star 13%, Leader Ambassador 14%, Leader Majestic sampai Executive Director 15%. Hak mentah member dihitung dari kaki terlemah: rate rank x PV terpasang/lebih kecil, maksimal Rp250 juta per member per bulan. Bonus Pasangan dibayar maksimal selama ruang payout pasangan masih masuk acuan 55%. Jika hak Bonus Pasangan sendiri melewati ruang payout tersebut, pembayaran pasangan disesuaikan proporsional ke penerima Bonus Pasangan."}</p></div>
...
function settingsView() {
  ...
  const canEditProfile = state.activeRole === "admin" || state.activeRole === "branch";
  ...
  return `
    <article class="card">
      <div class="toolbar">
        ...
        ${canEditProfile ? `<button class="primary" data-profile-edit="${attr(`${type}:${user.id}`)}">Ubah Data</button>` : `<span class="pill">Hanya lihat profil</span>`}
      </div>
...
function serverBonusComponentRows(bonus) {
  ...
  return rows
    ...
    .map(([label, amount]) => `<div class="bonus-row"><span>${html(label)}</span><b class="money">${money(amount)}</b></div>`)
    .join("") || `<p class="bonus-empty">Belum ada bonus di bonus ledger untuk tampilan ini.</p>`;
}
...
function serverBonusesView() {
  ...
  if (state.activeRole === "member") {
    const row = rows[0];
    return `
      ${controlPanel("Filter Bonus Saya", controls, `Sumber ledger: ${source.label}`)}
      <div style="height:16px"></div>
      <article class="card">
        <div class="toolbar">
        ...
        ${serverBonusPage.loading ? `<p class="muted">Memuat bonus database...</p>` : serverBonusPage.error ? `<p class="muted">${html(serverBonusPage.error)}</p>` : row ? `
        ...
        ` : `<p class="muted">Belum ada bonus pada ledger periode ini.</p>`}
      </article>
    `;
  }
  ...
  `).join("");
  return `
    ${controlPanel("Filter Bonus Admin", controls, `Sumber ledger: ${source.label}`)}
    <div style="height:16px"></div>
    <div class="grid kpi-grid">
      ${kpiCard("Data Tampil", rows.length, "50 data per halaman")}
      ${kpiCard("Total Bonus Tampil", money(totalBonus), "Dari bonus ledger")}
      ${kpiCard("Sudah Dibayar", `${paidRows.length}/${payableRows.length}`, "Status pembayaran periode ini")}
      ${kpiCard("Periode Ledger", monthLabel(source.ledgerPeriod), source.label)}
    </div>
    <article class="card compact-toolbar" style="margin-top:16px">
      ...
      ${serverBonusPage.loading ? `<p class="muted">Memuat bonus database...</p>` : serverBonusPage.error ? `<p class="muted">${html(serverBonusPage.error)}</p>` : ""}
    </article>
    <div class="grid three-col bonus-card-grid" style="margin-top:16px">
      ${!serverBonusPage.loading && !serverBonusPage.error ? cards || `<article class="card"><p class="muted">Belum ada bonus pada ledger periode ini.</p></article>` : ""}
    </div>
  `;
}
...
function bonusControlView() {
  ...
  return `
    ${controlPanel("Periode Tutup Buku", [
      monthControl("Bulan & Tahun")
    ], backendActive ? "Mode backend aktif: tutup buku akan menulis hasil ke bonus ledger database." : "Mode lokal: kontrol ini masih menghitung estimasi di browser.")}
    <div style="height:16px"></div>
    <div class="grid kpi-grid">
      ...
      ${kpiCard("Tutup Buku", backendActive ? "Backend" : "Lokal", selectedMonthLabel())}
    </div>
    <article class="card" style="margin-top:16px">
      <div class="toolbar">
        <div>
          <h3>Tutup Buku Backend</h3>
          <p class="muted">Proses ini menghitung bonus dari database dan menyimpan hasilnya ke bonus ledger.</p>
        </div>
        <div class="toolbar-actions">
          <button class="primary" data-run-server-bonus ${backendActive && !serverBonusRun.loading ? "" : "disabled"}>${serverBonusRun.loading ? "Menghitung..." : "Jalankan Tutup Buku Backend"}</button>
        </div>
      </div>
      ...
      <p class="notice">Jika dijalankan ulang untuk bulan yang sama, hasil bonus ledger bulan tersebut dihitung ulang agar tidak dobel.</p>
    </article>
    <article class="card" style="margin-top:16px">
      <div class="toolbar">
        <div>
  ...
  return withCalcMembers(members, () => {
    ...
    const targetRows = [
      ...
      return {
        ...
        target,
        paid: paidAmount,
        gap: active ? Math.max(0, target - paidAmount) : 0,
        inactiveReserve: active ? 0 : target,
        note: active ? note : `${label} sedang nonaktif massal di Kontrol Bonus; tidak dihitung sebagai dana kelola perusahaan.`
      };
    });
...
function aggregateForfeitureAuditData(year = selectedYearValue()) {
  ...
  return {
    ...
    totals: {
      ...
      safeManaged: Math.max(0, poolSummary.payoutGap - activeCarryReserve - Number(poolSummary.inactiveReserved || 0))
    }
  };
}
...
function attachViewEvents() {
    ...
    input.addEventListener("change", async () => {
      ...
      if (apiConnected() && state.activeRole === "admin") {
        try {
          ...
          state.bonusSettings = { ...defaultState.bonusSettings, ...(state.bonusSettings || {}), [key]: !checked };
          saveState();
          showToast(error.message || "Pengaturan bonus database gagal disimpan.");
          render();
          return;
        }
      }
  ...
  document.querySelectorAll("[data-server-add-member]").forEach((button) => {
    button.addEventListener("click", () => openDialog("serverMember"));
  });
  ...
  document.querySelectorAll("[data-bulk-delete]").forEach((button) => {
    button.addEventListener("click", () => deleteSelectedItems(button.dataset.bulkDelete));
  });
...
async function updateServerVisiblePayments(idsText, checked, ledgerPeriod, paymentMonth, paymentPeriod) {
  ...
  try {
    ...
    serverStockistPayouts.key = "";
    render();
    showToast(checked ? "Pembayaran member database berhasil diceklis." : "Ceklis pembayaran database dikosongkan.", "success");
  } catch (error) {
    showToast(error.message || "Status pembayaran database gagal disimpan.");
  }
}
...
async function updateServerStockistTransfer(stockistId, checked, payload = {}) {
  ...
  try {
    ...
    showToast(error.message || "Status dana stokis database gagal disimpan.");
  }
}
...
function seedCustomTestMembers() {
  if (state.activeRole !== "admin") {
...
function openDialog(type, id = null) {
  ...
  let item = (type === "omzet" || type === "member") && baseItem
    ...
    : baseItem;
  if (type === "serverMember" && id && item) item = { ...item, password: "", sponsor: item.sponsor || "", parent: item.parent || "", stockist: item.stockist || "" };
  if (type === "member" && item) {
    item = {
      ...item,
  ...
  dialogTitle.textContent = type === "omzet" || type === "serverOmzet"
    ...
    : type === "serverMember"
      ? id ? "Edit Member Database" : "Tambah Member Database"
      : type === "serverStockist"
        ? id ? "Edit Stokis Database" : "Tambah Stokis Database"
        : type === "serverAdmin"
          ? id ? "Edit Admin Database" : "Tambah Admin Database"
          : type === "serverAnnouncement"
            ? id ? "Edit Pengumuman Database" : "Tambah Pengumuman Database"
...
function selectField(field, value) {
  return `<select name="${attr(field.key)}" data-field-key="${attr(field.key)}">${field.options.map((option) => `<option value="${attr(option)}" ${option === value ? "selected" : ""}>${html(option)}</option>`).join("")}</select>`;
}
...
function fieldsFor(type) {
  ...
  if (type === "serverMember") {
    ...
    return fields.filter((field) => state.activeRole !== "stockist" || field.key !== "stockist");
  }
...
function blankItem(type) {
  ...
  if (type === "serverMember") {
    ...
    return { id: nextMemberId(), name: "", email: "", phone: "", sponsor: "Pusat", parent: "", stockist, ppv: 0, password: "ascendia" };
  }
  if (type === "member") {
    const stockist = state.activeRole === "stockist"
      ? state.activeId
...
async function saveServerDialog(data) {
  if (editTarget.type === "serverOmzet") {
    ...
    resetServerMembersPagination();
    render();
    showToast(`PV ${pv(pvAmount)} berhasil disimpan ke database.`, "success");
    return;
  }
  ...
  if (editTarget.type === "serverMember") {
    const payload = {
      ...
      rankName: editTarget.id ? undefined : "Member"
    };
    ...
    if (editTarget.id) {
      ...
      if (!updatePayload.rankName) delete updatePayload.rankName;
    ...
    resetServerMembersPagination();
    render();
    showToast(editTarget.id ? "Member database berhasil diperbarui." : "Member database berhasil ditambahkan.", "success");
  }
}
...
async function saveServerMasterDialog(data) {
  ...
  render();
  showToast(`${info.label} database berhasil disimpan.`, "success");
  return true;
}

async function archiveServerMember(memberId) {
  if (!confirm(`Arsipkan member ${memberId}? Data tidak dihapus permanen, hanya tidak tampil di daftar aktif.`)) return;
  try {
    await apiRequest(`/api/members/${encodeURIComponent(memberId)}`, { method: "DELETE" });
    resetServerMembersPagination();
    render();
    showToast("Member database berhasil diarsipkan.", "success");
  } catch (error) {
    showToast(error.message || "Member database gagal diarsipkan.");
  }
}
...
async function updateServerTupo(memberId, checked) {
  try {
    ...
    serverMembersPage.loadedKey = "";
    render();
    showToast(checked ? "TUPO database berhasil diceklis." : "TUPO database dikosongkan.", "success");
  } catch (error) {
    showToast(error.message || "TUPO database gagal disimpan.");
    serverTupoPage.loadedKey = "";
    render();
  }
}
...
async function archiveServerMaster(type, id) {
  ...
  try {
    ...
    render();
    showToast(`${label} database berhasil diarsipkan.`, "success");
  } catch (error) {
    showToast(error.message || `${label} database gagal diarsipkan.`);
  }
}
...
async function saveDialog() {
  ...
  if (editTarget.type === "serverOmzet" || editTarget.type === "serverMember") {
    try {
      ...
      showToast(error.message || "Data database gagal disimpan.");
    }
    return;
  }
