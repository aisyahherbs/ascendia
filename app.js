...
function serverStockistDashboardView() {
  ...
  if (scope === "year") {
    return `
      ...
      <article class="card">
        ...
        <p class="notice">Untuk data online, gunakan tampilan Periode Bayar atau 1 Bulan Full agar angka stokis diambil langsung dari database VPS. Rekap tahunan tidak memakai data contoh lokal.</p>
      </article>
    `;
  }
  ...
  return `
    ...
    <div style="height:16px"></div>
    ${loading ? `<article class="card"><p class="muted">Memuat dashboard stokis database...</p></article>` : error ? `<article class="card"><p class="muted">${html(error)}</p></article>` : `
      <div class="grid kpi-grid">
...
function serverMemberHomeView() {
  ...
  if (scope === "year") {
    return `
      ...
      <article class="card">
        <h3>Rekap 1 Tahun</h3>
        <p class="notice">Untuk akun online, gunakan tampilan Periode Bayar atau 1 Bulan Full agar angka diambil langsung dari database VPS. Rekap tahunan tidak memakai data contoh lokal.</p>
      </article>
    `;
  }
  ensureServerDashboardSummary();
  ...
  return `
    ${controlPanel("Filter Perkembangan Member", [
      ...
      memberBonusScopeControl("Tampilan"),
      scope === "payment" ? paymentPeriodControl("Periode Bayar") : ""
    ], `Sumber database: ${source.label}`)}
    <div style="height:16px"></div>
    ${loading ? `<article class="card"><p class="muted">Memuat perkembangan member database...</p></article>` : error ? `<article class="card"><p class="muted">${html(error)}</p></article>` : `
      <div class="grid kpi-grid">
        ...
        ${kpiCard("Bonus Diterima", money(bonus.rupiah || 0), "Sesuai ledger tutup buku")}
...
async function loadServerRevenuePage(key) {
  try {
    ...
    serverRevenuePage.error = error.message || "Data omset database belum bisa dimuat.";
    if (error.status === 401) {
      isAuthenticated = false;
      saveApiSession(null);
      sessionStorage.removeItem(SESSION_KEY);
...
function serverMembersView() {
  ...
  return `
    ...
    <article class="card">
      ...
      <p class="notice">Mode database aktif. Tambah, edit, arsip member, peringkat, TUPO, input PV, dan pengaturan bonus personal disimpan ke backend.</p>
    </article>
  `;
}
...
function serverStockistsView() {
  ...
  return `
    ...
    <article class="card">
      <div class="toolbar">
        <div>
          <h3>${isStockistView ? "Profil & Dana Stokis Saya" : state.activeRole === "branch" ? `Stokis Cabang ${html(currentUser()?.area || "")}` : "Data Stokis Database"}</h3>
          <p class="muted">${isStockistView ? "Menampilkan omset member, fee stokis, bonus member yang harus disalurkan, dan status dana dari pusat/cabang." : "Mode database: data stokis dimuat 50 baris per halaman, angka pembayaran diambil dari bonus ledger."}</p>
        </div>
        <div class="toolbar-actions">
...
function serverRevenueView() {
  ...
  `).join("");
  return `
    ${controlPanel("Filter Omset Database", [
      monthControl("Bulan & Tahun")
    ], isMemberView ? `Menampilkan omset pribadi ${selectedMonthLabel()} dari database VPS.` : `Input PV disimpan ke database VPS untuk ${selectedMonthLabel()}. Nilai yang diinput adalah total PV bulan terpilih.`)}
    <div style="height:16px"></div>
    <div class="grid kpi-grid">
      ${kpiCard("Bulan", selectedMonthLabel(), "Bulan pengecekan")}
    ...
    <article class="card" style="margin-top:16px">
      <div class="toolbar">
        <h3>${isMemberView ? "Omset Saya" : state.activeRole === "stockist" ? "Input Omset Member Stokis" : "Rincian Omset Database"}</h3>
        <div class="toolbar-actions">
          ${isMemberView ? "" : searchBox("revenue", "Cari ID, nama, peringkat")}
          <span class="pill">${members.length} data tampil</span>
      ...
      ${serverRevenuePage.loading ? `<p class="notice">Memuat omset dari database...</p>` : ""}
      ...
      <div class="table-wrap">
        <table>
          ...
          <tbody>${rows || emptyRow(serverRevenuePage.loading ? "Memuat data..." : "Belum ada data member di database.")}</tbody>
        </table>
      </div>
