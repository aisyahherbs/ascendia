...
function apiMemberToLocal(row) {
  return {
    ...
    pairLeftPv: Number(row.pair_left_basis_pv ?? row.pairLeftPv ?? row.left_pv ?? row.leftPv ?? 0),
    pairRightPv: Number(row.pair_right_basis_pv ?? row.pairRightPv ?? row.right_pv ?? row.rightPv ?? 0),
    pairVolumePv: Number(row.pair_volume_pv ?? row.pairVolumePv ?? 0),
    leftCarryPv: Number(row.left_carry_pv ?? row.leftCarryPv ?? 0),
    rightCarryPv: Number(row.right_carry_pv ?? row.rightCarryPv ?? 0),
    carryInLeft: Number(row.carry_in_left_pv ?? row.carryInLeft ?? 0),
    carryInRight: Number(row.carry_in_right_pv ?? row.carryInRight ?? 0),
    expiredCarryPv: Number(row.expired_carry_pv ?? row.expiredCarryPv ?? 0),
...
function calcRawPairBonus(member) {
  ...
  const pairedPv = Number(member.pairVolumePv ?? 0) || Math.min(member.pairLeftPv ?? member.leftPv ?? 0, member.pairRightPv ?? member.rightPv ?? 0);
...
function pairPoolInfo(members = calcMembers()) {
  ...
  const pairBonusFactor = rawPairSystemBv > availablePairSystemBv
    ? Math.max(0, Math.min(1, availablePairSystemBv / rawPairSystemBv))
    : 1;
  const maxPairPoolBv = Math.floor(rawPairBonusBv * pairBonusFactor);
  const overPairPoolBv = Math.max(0, rawPairSystemBv - availablePairSystemBv);
  const pairPoolUsage = availablePairSystemBv ? rawPairSystemBv / availablePairSystemBv : rawPairSystemBv > 0 ? Infinity : 0;
  const info = {
    companyTnpv,
    maxPayoutBv,
    referencePairPoolBv,
...
function calcPairSources(member) {
  ...
  const pairedPv = Number(member.pairVolumePv ?? 0) || Math.min(leftBasis, rightBasis);
...
function dashboardView() {
  ...
  const dashboardBonusData = withCalcMembers(members, () => {
    const bonusTotals = members.reduce((totals, item) => {
      ...
      const pairedPv = bonus.pair > 0 ? (Number(item.pairVolumePv || 0) || Math.min(item.pairLeftPv ?? item.leftPv ?? 0, item.pairRightPv ?? item.rightPv ?? 0)) : 0;
      totals.performance += bonus.performance * 1000;
      totals.pair += bonus.pair * 1000;
  ...
  const topPairRows = bonusTotals.rows
    ...
    .map(({ member, bonus, rawPair }) => `
      <tr>
        ...
        <td>${pv(Number(member.pairVolumePv || 0) || Math.min(member.pairLeftPv ?? member.leftPv ?? 0, member.pairRightPv ?? member.rightPv ?? 0))}</td>
        <td>${pv(member.pairLeftPv ?? member.leftPv ?? 0)} / ${pv(member.pairRightPv ?? member.rightPv ?? 0)}</td>
      </tr>
    `).join("");
...
function placementEmptySlotsHtml(member, children, depth) {
  const usedSides = new Set(children.map((child, index) => placementSideOf(child, index)));
  const labels = [
    !usedSides.has("L") ? "Kiri" : "",
    !usedSides.has("R") ? "Kanan" : ""
  ].filter(Boolean);
...
function placementScore(member) {
  const pairable = Number(member.pairVolumePv || 0) || Math.min(member.leftPv, member.rightPv);
