...
let menuVisibilityState = {
  ...
  loading: false,
  error: "",
  data: { admin: {}, branch: {}, stockist: {}, member: {} }
};
let serverTreeState = {
...
function roleNavItems(role = state.activeRole) {
  ...
  const visible = items.filter(([key]) => hidden[key] !== false);
...
async function loadMenuVisibility() {
  ...
  try {
    ...
    menuVisibilityState = {
      ...
      loading: false,
      error: "",
      data: { admin: {}, branch: {}, stockist: {}, member: {}, ...(result.data || {}) }
    };
  } catch (error) {
...
function serverMembersView() {
  ...
  return `
    ...
    <article class="card">
      ...
      <p class="notice">Tambah, edit, arsip member, peringkat, TUPO, input PV, dan pengaturan bonus personal disimpan langsung ke sistem.</p>
    </article>
  `;
}
  ...
  return `
    ...
    ${state.activeRole === "admin" ? `
      <article class="card" style="margin-top:16px">
        <div class="toolbar">
          <div>
            <h3>Kontrol Halaman Bawahan</h3>
            <p class="muted">Atur halaman mana yang boleh tampil untuk Admin Cabang, Stokis, dan Member.</p>
          </div>
...
function menuVisibilityControlsHtml() {
  ...
  return roles.map((role) => `
    <details class="detail-card" open>
      <summary>${html(roleName(role))}</summary>
      <div class="bonus-grid">
...
function attachViewEvents() {
  ...
  document.querySelectorAll("[data-menu-visibility]").forEach((input) => {
    input.addEventListener("change", async () => {
      ...
      const nextData = structuredClone(menuVisibilityState.data || { admin: {}, branch: {}, stockist: {}, member: {} });
...
function fieldsFor(type) {
  if (type === "serverStockist") return [
    ...
    { key: "newId", label: "Ubah ID Stokis", adminOnly: true },
    { key: "name", label: "Nama" },
  ...
  ].filter((field) => !field.adminOnly || state.activeRole === "admin");
  if (type === "serverAdmin") return [
    { key: "id", label: "ID Admin", readonly: Boolean(editTarget?.id) },
    { key: "newId", label: "Ubah ID Admin", adminOnly: true },
    { key: "name", label: "Nama" },
    { key: "email", label: "Email", inputType: "email" },
    { key: "phone", label: "Nomor HP" },
  ...
  if (type === "serverMember") {
    const fields = [
      ...
      { key: "newId", label: "Ubah ID Member", adminOnly: true },
      { key: "name", label: "Nama" },
      { key: "email", label: "Email", inputType: "email" },
      { key: "phone", label: "Nomor HP" },
...
async function archiveServerMember(memberId) {
  ...
  try {
    ...
    resetServerMembersPagination();
    render();
    showToast("Member berhasil diarsipkan.", "success");
  } catch (error) {
    showToast(error.message || "Member gagal diarsipkan.");
  }
}
