    ...
    <article class="card">
      <div class="toolbar">
        ...
        <div class="toolbar-actions">
          ...
          ${bulkActionButton("member", "Arsipkan Terpilih")}
...
function serverPlacementNodeMetrics(member, depth = 0, children = [], loadedChildren = true, knownChildCount = children.length) {
  const left = loadedChildren ? children.find((child) => child.placementSide === "L") || children[0] || null : null;
  const right = loadedChildren ? children.find((child) => child.placementSide === "R") || children.find((child) => child.id !== left?.id) || null : null;
  const visibleCount = loadedChildren ? children.length : knownChildCount;
  const recommendedSide = !loadedChildren && knownChildCount > 0
    ? "Buka cabang"
    : !left ? "Kiri" : !right ? "Kanan" : "Penuh";
  return `
    ...
    <div class="team-main placement-main">
      ...
      <div class="placement-leg-summary">
        ${!loadedChildren && knownChildCount > 0
          ? placementPendingLegSummary(knownChildCount)
          : `${placementMiniLeg("Kiri", left, left ? left.tnpv : 0, recommendedSide === "Kiri")}${placementMiniLeg("Kanan", right, right ? right.tnpv : 0, recommendedSide === "Kanan")}`}
      </div>
    </div>
  `;
}
...
function serverTreeNodeHtml(kind, memberId, depth = 0, visited = new Set()) {
  ...
  return `
    <div class="${branchClass} ${depth === 0 ? "root" : ""} ${hasChildren ? "has-children" : ""} ${isCollapsed ? "collapsed" : ""}" style="--depth:${depth}">
      <div class="${rowClass}">
        ${selectionPanelCell("member", member.id)}
...
function placementPendingLegSummary(childCount = 0) {
  const leftLabel = childCount >= 1 ? "Ada kaki" : "Slot kosong";
  const rightLabel = childCount >= 2 ? "Ada kaki" : "Slot kosong";
  return `
    <div class="placement-mini-leg pending">
      <span>Kiri</span>
      <b>${leftLabel}</b>
      <small>Buka cabang untuk detail</small>
    </div>
    <div class="placement-mini-leg ${childCount >= 2 ? "pending" : "empty"}">
      <span>Kanan</span>
      <b>${rightLabel}</b>
      <small>${childCount >= 2 ? "Buka cabang untuk detail" : "Siap dipakai"}</small>
    </div>
  `;
}
...
async function archiveSelectedServerMembers() {
  ...
  if (!ids.length) return;
  if (!confirm(`Arsipkan ${ids.length} member terpilih? Data tidak dihapus permanen dan bisa dipakai ulang sesuai aturan arsip.`)) return;
  try {
    for (const id of ids) {
    ...
    serverCompanyFunds.key = "";
    render();
    showToast(`${ids.length} member berhasil diarsipkan.`, "success");
  } catch (error) {
    showToast(error.message || "Member terpilih gagal diarsipkan.");
  }
}
