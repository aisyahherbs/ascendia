...
function openDialog(type, id = null) {
  ...
  let item = (type === "omzet" || type === "member") && baseItem
    ...
    : baseItem;
  if (type === "serverMember" && item) item = { ...item, rankName: item.rank || item.rankName || "Member", password: id ? "" : item.password || "ascendia", sponsor: item.sponsor || "Pusat", parent: item.parent || "", stockist: item.stockist || "" };
  if (type === "member" && item) {
    item = {
      ...item,
